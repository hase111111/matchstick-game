...
MenuUI::MenuUI(
    ...
    : dxlib_input_ptr_(dxlib_input_ptr),
      font48_handle_(dxlib_resource_loader_ptr->getFontHandle(
          language_record_ptr->getCurrentCountry(),
          "data/font/azuki_font48.dft")),
      font32_handle_(dxlib_resource_loader_ptr->getFontHandle(
          language_record_ptr->getCurrentCountry(),
          "data/font/azuki_font32.dft")),
      font24_handle_(dxlib_resource_loader_ptr->getFontHandle(
          language_record_ptr->getCurrentCountry(),
          "data/font/azuki_font24.dft")),
      select_sound_effect_handle_(dxlib_resource_loader_ptr->getSoundHandle(
          "data/sound/selecting3.mp3")),
      decision_sound_effect_handle_(dxlib_resource_loader_ptr->getSoundHandle(
          "data/sound/selecting2.mp3")),
  ...
  // 画像を読み込む
  bar_image_handle_map_[BarType::kGameStart] =
      dxlib_resource_loader_ptr->getImageHandle("data/img/icon_game.png");
  bar_image_handle_map_[BarType::kRule] =
      dxlib_resource_loader_ptr->getImageHandle("data/img/icon_rule.png");
  bar_image_handle_map_[BarType::kSetting] =
      dxlib_resource_loader_ptr->getImageHandle("data/img/icon_setting.png");
  bar_image_handle_map_[BarType::kReplay] =
      dxlib_resource_loader_ptr->getImageHandle("data/img/icon_replay.png");
  bar_image_handle_map_[BarType::kLanguage] =
      dxlib_resource_loader_ptr->getImageHandle("data/img/icon_language.png");
  ...
  // テキストを初期化
  bar_text_map_[BarType::kGameStart] =
      language_record_ptr->getValue("menu_game");
  bar_text_map_[BarType::kRule] = language_record_ptr->getValue("menu_rule");
  bar_text_map_[BarType::kSetting] =
      language_record_ptr->getValue("menu_setting");
  bar_text_map_[BarType::kReplay] =
      language_record_ptr->getValue("menu_replay");
  bar_text_map_[BarType::kLanguage] =
      language_record_ptr->getValue("menu_language");
}
...
bool MenuUI::update() {
  ...
  if (selected_bar_type_ != getBarTypeFromIndex()) {
    ...
    DEBUG_PRINT(std::format("index_x:={}, index_y:={}",
                            selected_bar_index_x_ % kIndexMaxX,
                            selected_bar_index_y_ % kIndexMaxY));
  }
...
void MenuUI::updateSelectedBarType() {
  if (dxlib_input_ptr_->getInputType() == DxLibInput::InputType::kKeyboard) {
    // 横方向の選択
    if (selected_bar_index_y_ % kIndexMaxY == 0 ||
        selected_bar_index_y_ % kIndexMaxY == 2) {
      ...
      } else if (dxlib_input_ptr_->getKeyboardPressingCount(KEY_INPUT_RIGHT) ==
                 1) {
        selected_bar_index_x_ = selected_bar_index_x_ % kIndexMaxX == 2 ? 0 : 2;
      }
    } else {
    ...
    } else if (dxlib_input_ptr_->getKeyboardPressingCount(KEY_INPUT_DOWN) ==
               1) {
    ...
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < kIndexMaxX; ++j) {
        ...
        if (x <= input_x && input_x <= x + kBarWidth && y <= input_y &&
            input_y <= y + kBarHeight) {
    ...
    if (kBarStartX <= input_x &&
        input_x <= kBarStartX + kBarWidth * 2 + kBarDistance &&
...
void MenuUI::drawBar() const {
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 3; ++j) {
      ...
      DrawBox(kBarStartX + (kBarWidth + kBarDistance) * j + kBarThickness,
              ...
              kBarStartY + (kBarHeight + kBarDistance) * i + kBarHeight -
                  kBarThickness,
              (bar_type == selected_bar_type_) ? bar_hovered_color
                                               : bar_inner_color,
              TRUE);
      ...
      DrawRotaGraph(
          kBarStartX + (kBarWidth + kBarDistance) * j + kBarWidth / 2,
      ...
      DrawStringToHandle(
          ...
          bar_text_map_.at(bar_type).c_str(), GetColor(0x00, 0x00, 0x00),
          font32_handle_);
    }
  }

  // 左上のバーは2つのバーを合わせたもの
  DrawBox(kBarStartX, kBarStartY, kBarStartX + kBarWidth * 2 + kBarDistance,
          kBarStartY + kBarHeight, bar_color, TRUE);

  DrawBox(kBarStartX + kBarThickness, kBarStartY + kBarThickness,
          kBarStartX + kBarWidth * 2 + kBarDistance - kBarThickness,
          kBarStartY + kBarHeight - kBarThickness,
          (BarType::kGameStart == selected_bar_type_) ? bar_hovered_color
                                                      : bar_inner_color,
          TRUE);
  ...
  DrawRotaGraph(kBarStartX + kBarWidth / 3 + kBarDistance / 2,
                kBarStartY + kBarHeight / 2, 1.0,
                bar_rotation_map_.at(BarType::kGameStart), image_handle, TRUE);

  // テキストを描画
  const int text_width = GetDrawStringWidthToHandle(
      bar_text_map_.at(BarType::kGameStart).c_str(),
      static_cast<int>(bar_text_map_.at(BarType::kGameStart).size()),
      font48_handle_);
  DrawStringToHandle(
      kBarStartX + kBarWidth * 5 / 4 + kBarDistance / 2 - text_width / 2,
      kBarStartY + kBarHeight / 3 - 20,
      bar_text_map_.at(BarType::kGameStart).c_str(), GetColor(0x00, 0x00, 0x00),
      font48_handle_);
}
...
void MenuUI::drawButton() const {
  ...
  // ボタンを描画
  for (int i = 0; i < 2; ++i) {
    DrawBox(GameConst::kResolutionX - kButtonWidth +
                i * (kButtonWidth + kBarThickness) - kButtonRight,
            GameConst::kResolutionY - kButtonBottom,
            GameConst::kResolutionX - kButtonWidth * 2 +
                i * (kButtonWidth + kBarThickness) - kButtonRight,
            GameConst::kResolutionY - kButtonBottom - kButtonHeight, bar_color,
            TRUE);
    ...
    DrawBox(
        GameConst::kResolutionX - kButtonWidth + i * button_width -
            kButtonRight - kBarThickness,
        GameConst::kResolutionY - kButtonBottom - kBarThickness,
        GameConst::kResolutionX - kButtonWidth * 2 + i * button_width -
            kButtonRight + kBarThickness,
    ...
    const int text_width = GetDrawStringWidthToHandle(
        text.c_str(), static_cast<int>(text.size()), font24_handle_);

    DrawStringToHandle(
        GameConst::kResolutionX - kButtonWidth * 2 +
            i * (kButtonWidth + kBarThickness) - kButtonRight +
            kButtonWidth / 2 - text_width / 2,
