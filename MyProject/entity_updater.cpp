...
void EntityUpdater::registerEntity(const std::shared_ptr<IEntity>& entity_ptr) {
  ...
  DEBUG_PRINT_INFO("register entity. now stack size is " +
...
void EntityUpdater::update() {
  ...
  for (size_t i = 0; i < entities_.size(); ++i) {
    ...
    if (!entities_[i]->update()) {
      ...
      DEBUG_PRINT_INFO("erase entity. now stack size is " +
                       std::to_string(entities_.size()));
    }
  }
}
...
void EntityUpdater::draw() const {
  // レイヤー順にソート
  // もし Entity が多い場合は Entity
  // 追加・削除時にソートして，その値を使いまわす設計に変更する
  std::vector<std::shared_ptr<IEntity>> sorted_entities_(entities_);

  auto sort_func = [](const std::shared_ptr<const IEntity>& a,
                      const std::shared_ptr<const IEntity>& b) {
