...
MenuScene::MenuScene(
  ...
  // フェードイン演出を追加
  const auto fade_effect_ptr =
      std::make_shared<FadeEffect>(30, FadeEffect::FadeType::kFadeIn, []() {});
...
void MenuScene::draw() const { entity_updater_ptr_->draw(); }
...
void MenuScene::initUI(
  ...
  // UI に渡すコールバック関数を設定
  const auto scene_change_callback = [this](const SceneName scene_name) {
    if (now_scene_change_) {
      return;
    }

    DEBUG_PRINT(std::format("Change the scene to {}.",
                            magic_enum::enum_name(scene_name)));

    now_scene_change_ = true;

    // フェードアウト演出を追加
    const auto fade_effect_ptr = std::make_shared<FadeEffect>(
        30, FadeEffect::FadeType::kFadeOut, [this, scene_name]() {
          scene_change_listener_ptr_->requestAddScene(scene_name,
  ...
  const auto dxlib_user_interface_base_ptr =
      std::make_shared<DxLibUserInterfaceBase>(dxlib_input_ptr);
  ...
  const auto to_game_button_ptr = std::make_shared<MenuButton>(
      ...
      [this, scene_change_callback]() {
        scene_change_callback(SceneName::kGame);
      },
      true);
  ...
  const auto to_rule_button_ptr = std::make_shared<MenuButton>(
      language_record_ptr, dxlib_resource_loader_ptr,
      ui_start_x + (ui_width + ui_interval) * 2 + ui_width / 2,
      ui_start_y + ui_height / 2, "data/img/icon_rule.png", "menu_rule",
      [this, scene_change_callback]() {
        scene_change_callback(SceneName::kRule);
      });
  ...
  const auto to_setting_button_ptr = std::make_shared<MenuButton>(
      language_record_ptr, dxlib_resource_loader_ptr, ui_start_x + ui_width / 2,
      ui_start_y + ui_height * 3 / 2 + ui_interval, "data/img/icon_setting.png",
      "menu_setting", [this, scene_change_callback]() {
        scene_change_callback(SceneName::kSetting);
      });
  ...
  const auto to_replay_button_ptr = std::make_shared<MenuButton>(
      language_record_ptr, dxlib_resource_loader_ptr,
      ui_start_x + (ui_width + ui_interval) + ui_width / 2,
      ui_start_y + ui_height * 3 / 2 + ui_interval, "data/img/icon_replay.png",
      "menu_replay", [this, scene_change_callback]() {
        scene_change_callback(SceneName::kReplay);
      });
  ...
  const auto to_language_button_ptr = std::make_shared<MenuButton>(
      language_record_ptr, dxlib_resource_loader_ptr,
      ui_start_x + (ui_width + ui_interval) * 2 + ui_width / 2,
      ui_start_y + ui_height * 3 / 2 + ui_interval,
      "data/img/icon_language.png", "menu_language",
      [this, scene_change_callback]() {
        scene_change_callback(SceneName::kLanguage);
      });
  ...
  const auto back_button_ptr = std::make_shared<SimpleBoxButton>(
      ...
      GameConst::kResolutionY - button_height / 2 - button_diff, button_width,
      button_height, "menu_back", "data/font/azuki_font24.dft", [this]() {
        if (now_scene_change_) {
          return;
        }
  ...
  const auto game_end_button_ptr = std::make_shared<SimpleBoxButton>(
      ...
      GameConst::kResolutionY - button_height / 2 - button_diff, button_width,
      button_height, "menu_end", "data/font/azuki_font24.dft",
      [this]() { game_end_ = true; });
