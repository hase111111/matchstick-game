...
SimpleBoxButton::SimpleBoxButton(
    ...
    const std::string& text_key, const std::string& font_name,
    const std::function<void()>& callback_when_clicked)
    : font_handle_(dxlib_resource_loader->getFontHandle(
          language_record->getCurrentCountry(), font_name)),
      click_sound_handle_(
...
void SimpleBoxButton::callbackWhenHoverEnded() { is_hovered_ = false; }
...
void SimpleBoxButton::draw() const {
  ...
  DxLib::DrawBox(center_x_ - width_ / 2 + button_thickness_,
                 ...
                 center_y_ + height_ / 2 - button_thickness_, button_color,
                 TRUE);
  ...
  DxLib::DrawStringToHandle(center_x_ - text_width / 2,
                            center_y_ - text_height / 2, text_.c_str(),
                            button_text_color_, font_handle_);
}
