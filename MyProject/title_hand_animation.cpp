...
TitleHandAnimation::TitleHandAnimation(
...
bool TitleHandAnimation::update() {
  ...
  for (auto& i : hand_) {
    ...
    if (i.y + hand_size_y_ / 2.0 < 0.0) {
      i.x = i.base_x + static_cast<double>(GetRand(random_range)) -
            random_range / 2.0;
      ...
      i.v = Hand::kMinVelocity +
            (Hand::kMaxVelocity - Hand::kMinVelocity) *
...
void TitleHandAnimation::draw() const {
  for (const auto& i : hand_) {
    // 画面上部に行くほど透明になる
    const int blend =
        150 - static_cast<int>(128 * (GameConst::kResolutionY - i.y) /
                               GameConst::kResolutionY);
    SetDrawBlendMode(DX_BLENDMODE_ALPHA, blend);

    // 描画
    const int draw_x = static_cast<int>(i.x);
    const int draw_y = static_cast<int>(i.y);
    DrawRotaGraph(draw_x, draw_y, 1.0, 0.0, image_handle_.at(i.graphic_index),
                  TRUE);
  }
...
std::array<int, TitleHandAnimation::kImageNum> TitleHandAnimation::loadImages(
  ...
  for (unsigned int i = 0; i < kImageNum; ++i) {
    image_handle[i] = dxlib_resource_loader_ptr->getImageHandle(
        std::format("data/img/hand/{}_hand.png", i));
  }
...
void TitleHandAnimation::initHandPos() {
  ...
  for (int i = 0; i < hand_num; ++i) {
    ...
    add.x = add.base_x =
        static_cast<double>(GameConst::kResolutionX) / hand_num * i +
        (hand_size_x_ / 2.0) + 10;
