...
bool TitleHandAnimation::update() {
    ...
    for (auto& i : hand_) {
        ...
        if (i.y + hand_size_y_ / 2.0 < 0.0) {
            ...
            // 画面外に設置（0～腕2分程度ずらす）
            i.y = static_cast<double>(GameConst::kWindowSizeY) + hand_size_y_ / 2.0;
...
void TitleHandAnimation::draw() const {
    for (const auto& i : hand_) {
        // 画面上部に行くほど透明になる
        const int blend = 150 - static_cast<int>(128 * (GameConst::kWindowSizeY - i.y) / GameConst::kWindowSizeY);
...
void TitleHandAnimation::initHandPos() {
    ...
    for (int i = 0; i < hand_num; ++i) {
        ...
        add.x = add.base_x = static_cast<double>(GameConst::kWindowSizeX) / hand_num * i + (hand_size_x_ / 2.0) + 10;

        // 画面外に設置（0～腕6分程度ずらす）
        add.y = static_cast<double>(GameConst::kWindowSizeY) + hand_size_y_ / 2.0 +
