...
MenuButton::MenuButton(
    ...
    int center_x, int center_y, const std::string& icon_path,
    const std::string& text_key,
    const std::function<void()>& callback_when_clicked,
    const bool is_large_button)
    : font_handle_(dxlib_resource_loader->getFontHandle(
          language_record->getCurrentCountry(),
          is_large_button ? "data/font/azuki_font48.dft"
                          : "data/font/azuki_font32.dft")),
      icon_image_handle_(dxlib_resource_loader->getImageHandle(icon_path)),
      click_sound_handle_(
          dxlib_resource_loader->getSoundHandle("data/sound/selecting2.mp3")),
      hover_sound_handle_(
          dxlib_resource_loader->getSoundHandle("data/sound/selecting3.mp3")),
      center_x_(center_x),
      center_y_(center_y),
...
bool MenuButton::isHovered(const int mouse_x, const int mouse_y) const {
  return (
      center_x_ - width_ / 2 <= mouse_x && mouse_x <= center_x_ + width_ / 2 &&
      center_y_ - height_ / 2 <= mouse_y && mouse_y <= center_y_ + height_ / 2);
}

void MenuButton::initHoverState(bool is_hovered) { is_hovered_ = is_hovered; }
...
void MenuButton::callbackWhenHoverEnded() { is_hovered_ = false; }
...
void MenuButton::draw() const {
  ...
  DxLib::DrawBox(center_x_ - width_ / 2 + button_thickness_,
                 center_y_ - height_ / 2 + button_thickness_,
                 center_x_ + width_ / 2 - button_thickness_,
                 center_y_ + height_ / 2 - button_thickness_,
                 is_hovered_ ? button_hovered_color_ : button_base_color_,
                 TRUE);

  if (is_large_button_) {
    // アイコンの描画
    DxLib::DrawRotaGraph(center_x_ - width_ * 1 / 3, center_y_, 1.0,
                         icon_rotation_, icon_image_handle_, TRUE);
    ...
    DxLib::DrawStringToHandle(center_x_ + width_ / 3 - text_width,
                              center_y_ - height_ / 10 - text_height / 2,
                              text_.c_str(), button_text_color_, font_handle_);
  } else {
    // アイコンの描画
    DxLib::DrawRotaGraph(center_x_, center_y_ + height_ / 6, 0.8,
                         icon_rotation_, icon_image_handle_, TRUE);
    ...
    DxLib::DrawStringToHandle(center_x_ - text_width / 2,
