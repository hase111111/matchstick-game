...
void MatchGameField::attack(const MatchGameRule& rule, const int attack_index,
                            const int attacked_index) {
  ASSERT(0 <= attack_index && attack_index < kMaxHand, "The index is invalid.");
  ASSERT(0 <= attacked_index && attacked_index < kMaxHand,
  ...
  // 攻撃と防御の手札を取得する．
  int& attack =
      player1_turn ? player1_hand[attack_index] : player2_hand[attack_index];
  int& defense = player1_turn ? player2_hand[attacked_index]
                              : player1_hand[attacked_index];
  ...
  if (rule.cut_off_valid) {
    ...
    // cut_off_number で割った余りを求める．
    // 10で割った場合は，5ちょうどになる場合0にする．
...
void MatchGameField::changeNegativeNumber(const MatchGameRule& rule,
  ...
  int& hand =
      player1_turn ? player1_hand[hand_index] : player2_hand[hand_index];
...
void MatchGameField::selfHarm(const MatchGameRule& rule,
                              const int attack_hand_index,
                              const int attacked_hand_index) {
  ASSERT(rule.self_harm_valid, "The self harm is invalid.");
  ASSERT(0 <= attack_hand_index && attack_hand_index < kMaxHand,
         "The index is invalid.");
  ASSERT(0 <= attacked_hand_index && attacked_hand_index < kMaxHand,
         "The index is invalid.");

  int& attack = player1_turn ? player1_hand[attack_hand_index]
                             : player2_hand[attack_hand_index];
  int& defense = player1_turn ? player1_hand[attacked_hand_index]
                              : player2_hand[attacked_hand_index];
...
std::vector<MatchGameField> MatchGameField::createFreeSortFieldList(
    const MatchGameRule& rule) const {
...
std::string MatchGameField::Move::toString() const {
  switch (type) {
    ...
    case match_stick::MatchGameField::Move::Type::kAttack: {
      return "Attack: " + std::to_string(attack_index) + " -> " +
             std::to_string(attacked_index);
    }
    case match_stick::MatchGameField::Move::Type::kChangeNegativeNumber: {
      return "ChangeNegativeNumber: " +
             std::to_string(change_negative_number_index);
    }
    case match_stick::MatchGameField::Move::Type::kSelfHarm: {
      return "SelfHarm: " + std::to_string(attack_index) + " -> " +
             std::to_string(attacked_index);
    }
