...
RuleUI::RuleUI(
    ...
    : on_button_pressed_(on_button_pressed),
      ...
      rule_ui_hexagon_(language_record_ptr, dxlib_input_ptr,
...
void RuleUI::drawButton() const {
  ...
  const int button_text_width = GetDrawStringWidthToHandle(
      button_text_.c_str(), static_cast<int>(button_text_.size()),
      font24_handle_);

  if (is_button_hovered_) {
    DrawBox(button_x, button_y, button_x + button_width,
            button_y + button_height, GetColor(50, 50, 50), TRUE);
    DrawStringToHandle(button_x + (button_width - button_text_width) / 2 - 5,
                       button_y + button_height / 2 - 10, button_text_.c_str(),
                       GetColor(255, 255, 255), font24_handle_);
  } else {
    DrawStringToHandle(button_x + (button_width - button_text_width) / 2 - 5,
                       button_y + button_height / 2 - 10, button_text_.c_str(),
  ...
  DrawBoxAA(static_cast<float>(button_x), static_cast<float>(button_y),
            ...
            static_cast<float>(button_y + button_height), GetColor(0, 0, 0),
            FALSE, 3.0);
}
...
void RuleUI::updateButton() {
  if (dxlib_input_ptr_->getInputType() == DxLibInput::InputType::kMouse) {
    ...
    if (is_button_hovered_ &&
        dxlib_input_ptr_->getMousePressingCount(MOUSE_INPUT_LEFT) == 1) {
