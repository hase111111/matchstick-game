...
void BgmPlayer::playBGM(const std::string& bgm_file) {
  ASSERT(isLoaded(bgm_file),
         "BGM is not loaded. File Path : " + bgm_file +
...
void BgmPlayer::changeAllVolume(const int volume) {
  ASSERT(0 <= volume && volume <= 100,
         "Volume must be in the range of 0 to 100.");

  volume_percent_ = volume;

  if (current_bgm_.has_value()) {
    ChangeVolumeSoundMem(255 * volume_percent_ / 100,
                         bgm_handle_[current_bgm_.value()]);
  }
}
