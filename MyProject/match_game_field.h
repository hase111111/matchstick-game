...
struct MatchGameField final {
  ...
  constexpr int getAliveHandNum(const bool player1) const {
    ...
    for (int i = 0; i < kMaxHand; ++i) {
      if ((player1 && player1_hand[i] > 0) ||
          (!player1 && player2_hand[i] > 0)) {
        ++num;
      }
    }
  ...
  //! @brief 初期盤面を生成する．
  static constexpr MatchGameField createInitialField(
      const MatchGameRule& rule) {
  ...
  //! @brief 次の盤面を生成する．
  std::vector<std::tuple<MatchGameField, Move>> createNextFieldList(
  ...
  void attack(const MatchGameRule& rule, int attack_player_hand_index,
              int attacked_player_hand_index);

  //! @brief プレイヤーの手を負の数にする．
  void changeNegativeNumber(const MatchGameRule& rule, int hand_index);

  //! @brief 自傷を行う．
  void selfHarm(const MatchGameRule& rule, int attack_hand_index,
                int attacked_hand_index);

  //! @brief 采配を行った後の盤面を生成する．
  std::vector<MatchGameField> createFreeSortFieldList(
      const MatchGameRule& rule) const;
};
