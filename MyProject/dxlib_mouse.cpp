...
DxLibMouse::DxLibMouse()
    : kMouseKeyCodes{MOUSE_INPUT_RIGHT, MOUSE_INPUT_LEFT, MOUSE_INPUT_MIDDLE,
                     MOUSE_INPUT_4,     MOUSE_INPUT_5,    MOUSE_INPUT_6,
                     MOUSE_INPUT_7,     MOUSE_INPUT_8},
      cursor_pos_x_(0),
      cursor_pos_y_(0),
...
double DxLibMouse::getDiffPos() const {
  return sqrt(static_cast<double>(getDiffPosY() * getDiffPosY() +
                                  getDiffPosX() * getDiffPosX()));
}
...
static_assert(MOUSE_INPUT_LEFT == 0x01,
              "DxLib mouse left button definition has changed.");
static_assert(MOUSE_INPUT_RIGHT == 0x02,
              "DxLib mouse right button definition has changed.");
static_assert(MOUSE_INPUT_MIDDLE == 0x04,
              "DxLib mouse middle button definition has changed.");
