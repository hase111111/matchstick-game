...
DebugScene::DebugScene(
  ...
  while (true) {
    ...
    DEBUG_PRINT(std::get<0>(next_field_list[index]).toString() + " " +
                std::get<1>(next_field_list[index]).toString());
...
void match_stick::DebugScene::onReturnFromOtherScene(
    const SceneChangeParameter&) {}
