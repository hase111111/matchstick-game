...
// このファイルを利用するためには，Visual Studioの設定で，
// C++→プリプロセッサ→標準対応のプリプロセッサを有効にする必要がある．
...
//! @def MATCH_STICK_SUB_MACRO_FIND_MEMBER_VARIABLE_FROM_VALUE
//! @brief MATCH_STICK_DEFINE_CONVERSION_NON_INTRUSIVEの補助マクロ．
//! 他のファイルから呼び出さないこと．
//! @n tomlファイルからクラスのメンバ変数を取得する．
//! @param VAR_NAME 変数名．
#define MATCH_STICK_SUB_MACRO_FIND_MEMBER_VARIABLE_FROM_VALUE(VAR_NAME)      \
{                                                                          \
    const std::string table_str = desc.VAR_NAME.table_name;                \
                                                                           \
    if (table_str == ::match_stick::toml_func::Toml11Description::kNoTable)  \
    {                                                                      \
        obj.VAR_NAME =                                                     \
            ::match_stick::toml_func::GetTomlValue<decltype(obj.VAR_NAME)>(  \
                v_,                                                        \
...
//! @def MATCH_STICK_SUB_MACRO_ASSIGN_MEMBER_VARIABLE_TO_VALUE
//! @brief MATCH_STICK_DEFINE_CONVERSION_NON_INTRUSIVEの補助マクロ．
//! 他のファイルから呼び出さないこと．
//! @n クラスのメンバ変数を tomlファイルに追加する．
//! @param VAR 変数名．
#define MATCH_STICK_SUB_MACRO_ASSIGN_MEMBER_VARIABLE_TO_VALUE(VAR)                  \
if (desc.VAR.table_name != ::match_stick::toml_func::Toml11Description::kNoTable) { \
    if (v.count(desc.VAR.table_name) == 0)                                        \
    {                                                                             \
        v[desc.VAR.table_name] = toml::table{};                                   \
    }                                                                             \
                                                                                  \
    ::match_stick::toml_func::SetTomlValue(&v[desc.VAR.table_name],                 \
        TOML11_STRINGIZE(VAR), obj.VAR);                                          \
}                                                                                 \
else                                                                              \
{                                                                                 \
    ::match_stick::toml_func::SetTomlValue(                                         \
        &v, TOML11_STRINGIZE(VAR), obj.VAR);                                      \
}


//! @def MATCH_STICK_SUB_MACRO_ADD_COMMENT
//! @brief MATCH_STICK_DEFINE_CONVERSION_NON_INTRUSIVEの補助マクロ．
//! 他のファイルから呼び出さないこと．
//! @n tomlファイルの要素にクラスの説明を追加する．
//! もし，説明の用意がなければ説明を追加しない．
//! @param VN 変数名．
#define MATCH_STICK_SUB_MACRO_ADD_COMMENT(VN)                                        \
if (desc.VN.description != "") {                                                   \
    if (desc.VN.table_name != ::match_stick::toml_func::Toml11Description::kNoTable) \
    {                                                                              \
...
//! @def MATCH_STICK_TOML11_DESCRIPTION_CLASS
//! @brief tomlファイルに説明を追加するためのクラスの宣言を行うためのマクロ．
//! @param CLASS クラス名．
#define MATCH_STICK_TOML11_DESCRIPTION_CLASS(CLASS) \
struct CLASS##Description final


//! @def MATCH_STICK_TOML11_FILE_NO_DESCRIPTION
//! @brief tomlファイルにファイルの説明を追加しないことを示す文字列．
//! @n MATCH_STICK_TOML11_DESCRIPTION_CLASS内に必ず記述する必要がある．
#define MATCH_STICK_TOML11_FILE_NO_DESCRIPTION() \
const ::std::vector<::std::string> file_description_vec{};

//! @def MATCH_STICK_TOML11_FILE_ADD_DESCRIPTION
//! @brief tomlファイルにファイルの説明を追加するためのマクロ．
//! @n MATCH_STICK_TOML11_DESCRIPTION_CLASS内に必ず記述する必要がある．
//! @param DESCRIPTION 説明．文字列で指定する．
#define MATCH_STICK_TOML11_FILE_ADD_DESCRIPTION(DESCRIPTION) \
const ::std::vector<::std::string> file_description_vec{sjis_to_utf8(DESCRIPTION)};

//! @def MATCH_STICK_TOML11_FILE_ADD_DESCRIPTION_MULTI_LINE
//! @brief tomlファイルにファイルの説明を追加するためのマクロ．
//! @n MATCH_STICK_TOML11_DESCRIPTION_CLASS内に必ず記述する必要がある．
//! @param DESCRIPTION_VEC 説明．文字列の vector で指定する．
#define MATCH_STICK_TOML11_FILE_ADD_DESCRIPTION_MULTI_LINE(DESCRIPTION_VEC) \
const ::std::vector<::std::string> file_description_vec = \
    ::match_stick::toml_func::sjis_to_utf8_vec(DESCRIPTION_VEC);


//! @def MATCH_STICK_TOML11_TABLE_ADD_DESCRIPTION
//! @brief tomlファイルにテーブルの説明を追加するためのマクロ．
//! @n MATCH_STICK_TOML11_DESCRIPTION_CLASS内に必ず記述する必要がある．
//! @param ... テーブル名と説明．
#define MATCH_STICK_TOML11_TABLE_ADD_DESCRIPTION(...) \
const std::vector<std::string> table_name_description_vec = \
    ::match_stick::toml_func::sjis_to_utf8_vec({ __VA_ARGS__ });

//! @def MATCH_STICK_TOML11_TABLE_NO_DESCRIPTION
//! @brief tomlファイルに追加するテーブルにコメントを追加しないことを示すマクロ．
//! @n MATCH_STICK_TOML11_DESCRIPTION_CLASS内に必ず記述する必要がある．
#define MATCH_STICK_TOML11_TABLE_NO_DESCRIPTION() \
const std::vector<std::string> table_name_description_vec = {};


//! @def MATCH_STICK_TOML11_VARIABLE_ADD_DESCRIPTION
//! @brief tomlファイルに変数とファイルの説明を追加するためのマクロ．
//! @param VARIABLE 変数名．
//! @param TABLE テーブル名．
//! @param DESCRIPTION 説明．
#define MATCH_STICK_TOML11_VARIABLE_ADD_DESCRIPTION(VARIABLE, TABLE, DESCRIPTION) \
const ::match_stick::toml_func::Toml11Description VARIABLE{                       \
    TABLE, sjis_to_utf8(DESCRIPTION)                                            \
}

//! @def MATCH_STICK_TOML11_VARIABLE_NO_DESCRIPTION
//! @brief ファイルの説明を追加したくない場合には，このマクロで変数を追加する．
//! @param VARIABLE 変数名．
//! @param TABLE テーブル名．
#define MATCH_STICK_TOML11_VARIABLE_NO_DESCRIPTION(VARIABLE, TABLE) \
const ::match_stick::toml_func::Toml11Description VARIABLE{TABLE, ""}

//! @def MATCH_STICK_TOML11_NO_TABLE
//! @brief tomlファイルに追加する変数をテーブルに追加しないことを示すためのマクロ．
#define MATCH_STICK_TOML11_NO_TABLE \
::match_stick::toml_func::Toml11Description::kNoTable

//! @def MATCH_STICK_TOML11_SERIALIZE
...
//! 必ずMATCH_STICK_TOML11_DESCRIPTION_CLASSを用意する必要がある．
//! @details 以下のように使用する．
//! @code
...
//! };
//!
//! MATCH_STICK_TOML11_DESCRIPTION_CLASS(Sample)
//! {
...
//!
//!     MATCH_STICK_TOML11_VARIABLE_ADD_DESCRIPTION(data, "number", "This is data");
//!     MATCH_STICK_TOML11_VARIABLE_ADD_DESCRIPTION(
//!         str, MATCH_STICK_TOML11_NO_TABLE, "This is str");
//!     MATCH_STICK_TOML11_VARIABLE_ADD_DESCRIPTION(
//!         enum_data, "enum", "This is enum_data");
//! };
//!
//! MATCH_STICK_TOML11_SERIALIZE(Sample, data, str, enum_data);
//! @endcode
//! @param NAME クラス名．クラスの型を指定する．
//! @param ... クラスのメンバ変数．過不足なく，全て指定する必要がある．
//! 可変長引数なので複数指定することができる．
#define MATCH_STICK_TOML11_SERIALIZE(NAME, ...)                                   \
namespace toml                                                                  \
{                                                                               \
template<>                                                                      \
struct from<NAME>                                                               \
{                                                                               \
  ...
  {                                                                             \
    ...
    TOML11_FOR_EACH_VA_ARGS(MATCH_STICK_SUB_MACRO_FIND_MEMBER_VARIABLE_FROM_VALUE, __VA_ARGS__)       \
    return obj;                                                                 \
  }                                                                             \
};                                                                              \
                                                                                \
  ...
  {                                                                             \
    ...
    {                                                                           \
      ...
      ++i;                                                                      \
                                                                                ...
                                                                                \
    TOML11_FOR_EACH_VA_ARGS(MATCH_STICK_SUB_MACRO_ASSIGN_MEMBER_VARIABLE_TO_VALUE, __VA_ARGS__)       \
    TOML11_FOR_EACH_VA_ARGS(MATCH_STICK_SUB_MACRO_ADD_COMMENT, __VA_ARGS__)                           \
    return v;                                                                   \
  }                                                                             \
};                                                                              \
                                                                                \
