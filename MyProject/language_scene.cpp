...
LanguageScene::LanguageScene(
  ...
  // エンティティの登録
  entity_updater_ptr_->registerEntity(std::make_shared<LanguageBackGroundBase>(
  ...
  const auto on_back_button_clicked = [this]() {
    ...
    entity_updater_ptr_->registerEntity(std::make_shared<FadeEffect>(
  ...
  entity_updater_ptr_->registerEntity(std::make_shared<LanguageUI>(
      ...
      std::bind(&LanguageScene::callBackOnBackButtonClicked, this,
                std::placeholders::_1)));
  ...
  const auto fade_effect_ptr =
      std::make_shared<FadeEffect>(60, FadeEffect::FadeType::kFadeIn, []() {});
...
void LanguageScene::callBackOnBackButtonClicked(const bool back_one_scene) {
  ...
  if (back_one_scene) {
    ...
    entity_updater_ptr_->registerEntity(std::make_shared<FadeEffect>(
        30, FadeEffect::FadeType::kFadeOut, all_delete_func));
  }
}
