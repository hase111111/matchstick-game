...
int FontLoader::getFontHandle(const Country country,
                              const std::string& file_path) const {
  ASSERT(alreadyLoadedFont(country, file_path),
         "The font has not been loaded yet : " + file_path);

  const int result_handle = isCountryForOtherLanguage(country)
                                ? font_handle_for_other_language_.at(file_path)
                                : font_handle_.at(file_path);

  ASSERT(result_handle >= 0,
         "Failed to get font handle. File Path : " + file_path);

  return result_handle;
}

void FontLoader::loadFontHandle(const Country country,
...
bool FontLoader::alreadyLoadedFont(const Country country,
                                   const std::string& file_path) const {
  if (isCountryForOtherLanguage(country)) {
