...
class SoundEffectLoader final {
 public:
  ...
  [[nodiscard]] inline int getSoundHandle(const std::string& file_path) const {
    ASSERT(
        isLoaded(file_path),
        "The sound effect is not loaded. File Path : " + file_path +
  ...
  //! @brief SEの音量を取得する．
  //! @return SEの音量．
  [[nodiscard]] inline int getVolumePercent() const { return volume_percent_; }

 private:
  [[nodiscard]] inline bool isLoaded(const std::string& file_path) const {
    return sound_handle_.contains(file_path);
  }

  std::map<std::string, int>
      sound_handle_;  //!< SEを格納する配列．キーはファイルパス．
