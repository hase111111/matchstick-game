...
class DxLibUserInterfaceBase final : public IEntity {
 public:
  explicit DxLibUserInterfaceBase(
      const std::shared_ptr<const DxLibInput>& dxlib_input);
  ~DxLibUserInterfaceBase() = default;

  void registerInterface(
      const std::shared_ptr<IDxLibUserInterface>& dxlib_interface, int id);

  void registerInterfaceDeployment(int target_id, int up_id, int down_id,
  ...
  int getLayer() const override { return constants::kUIFrontLayer; }
