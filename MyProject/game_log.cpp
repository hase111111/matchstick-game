...
void GameLog::draw() const {
    ...
    //盤面状況のアナウンス
    {
        ...
        DrawFormatStringToHandle(_centerx - temp / 2, Define::kWindowSizeY * 3 / 64, GetColor(0, 0, 0), small_font_handle_, "=現在の盤面=");
    }
    ...
    //ターン
    {
        ...
        DrawFormatStringToHandle(_centerx - temp / 2, Define::kWindowSizeY * 7 / 64, GetColor(0, 0, 0), small_font_handle_, temp_str.c_str());
    }
    ...
    //盤面状況
    {
        ...
        temp = GetDrawStringWidthToHandle(temp_str.c_str(), (int)temp_str.size(), small_font_handle_);
        DrawFormatStringToHandle(_centerx - temp / 2, Define::kWindowSizeY * 12 / 64, GetColor(0, 0, 0), small_font_handle_, temp_str.c_str());

        if (true) { temp_str = "あいて(後手) [" + std::to_string(0) + "," + std::to_string(0) + "]"; } else { temp_str = "あいて(先手) [" + std::to_string(0) + "," + std::to_string(0) + "]"; }
        if (true) { temp_str += ",采配残り" + std::to_string(0); }
        temp = GetDrawStringWidthToHandle(temp_str.c_str(), (int)temp_str.size(), small_font_handle_);
        DrawFormatStringToHandle(_centerx - temp / 2, Define::kWindowSizeY * 15 / 64, GetColor(0, 0, 0), small_font_handle_, temp_str.c_str());
    }
    ...
    //特殊ルール
    {
        ...
        temp = GetDrawStringWidthToHandle(temp_str.c_str(), (int)temp_str.size(), small_font_handle_);
        DrawFormatStringToHandle(_centerx - temp / 2, Define::kWindowSizeY * 21 / 64, GetColor(0, 0, 0), small_font_handle_, temp_str.c_str());

        temp_str = "自傷 ルール...";
        if (true) { temp_str += "あり"; } else { temp_str += "なし"; }
        temp = GetDrawStringWidthToHandle(temp_str.c_str(), (int)temp_str.size(), small_font_handle_);
        DrawFormatStringToHandle(_centerx - temp / 2, Define::kWindowSizeY * 24 / 64, GetColor(0, 0, 0), small_font_handle_, temp_str.c_str());
    }
}
