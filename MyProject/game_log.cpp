...
GameLog::GameLog(const std::shared_ptr<const LanguageRecord>& language_record_ptr,
    ...
    small_font_handle_(font_loader->loadAndGetFontHandle("data/font/PixelMplus10_size16.dft")) {
}
...
void GameLog::draw() const {
    ...
    //盤面状況
    {
        if (true) { temp_str = "あなた(先手)[" + std::to_string(0) + "," + std::to_string(0) + "]"; }
        else { temp_str = "あなた(後手)[" + std::to_string(0) + "," + std::to_string(0) + "]"; }
        ...
        if (true) { temp_str = "あいて(後手) [" + std::to_string(0) + "," + std::to_string(0) + "]"; }
        else { temp_str = "あいて(先手) [" + std::to_string(0) + "," + std::to_string(0) + "]"; }
        if (true) { temp_str += ",采配残り" + std::to_string(0); }
        temp = GetDrawStringWidthToHandle(temp_str.c_str(), (int)temp_str.size(), small_font_handle_);
