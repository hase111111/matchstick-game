...
constexpr int kTableColumnNum = static_cast<int>(
    magic_enum::enum_count<::match_stick::LanguageRecord::Country>());
...
constexpr int kButtonLeftX =
    ::match_stick::GameConst::kResolutionX - kButtonWidth * 3 / 2;
constexpr int kButtonTopY =
    ::match_stick::GameConst::kResolutionY - 2 * kButtonHeight - 50;
...
LanguageUI::LanguageUI(
    ...
    : dxlib_input_ptr_(dxlib_input),
      ...
      font32_handle_(dxlib_resource_loader_ptr->getFontHandle(
          current_country_, "data/font/azuki_font32.dft")),
      font24_handle_(dxlib_resource_loader_ptr->getFontHandle(
          current_country_, "data/font/azuki_font24.dft")),
      font20_handle_(dxlib_resource_loader_ptr->getFontHandle(
          current_country_, "data/font/azuki_font20.dft")),
      select_sound_handle_(dxlib_resource_loader_ptr->getSoundHandle(
          "data/sound/selecting3.mp3")),
      decide_sound_handle_(dxlib_resource_loader_ptr->getSoundHandle(
          "data/sound/selecting2.mp3")),
      checked_img_handle_(dxlib_resource_loader_ptr->getImageHandle(
          "data/img/icon_checked.png")),
      ...
      button_apply_back_text_(
          language_record_ptr->getValue("language_apply_and_back")),
...
void LanguageUI::updateSelectIndex() {
  if (dxlib_input_ptr_->getInputType() == DxLibInput::InputType::kKeyboard) {
    ...
    } else if (dxlib_input_ptr_->getKeyboardPressingCount(KEY_INPUT_LEFT) ==
               1) {
      PlaySoundMem(select_sound_handle_, DX_PLAYTYPE_BACK);
      select_index_x_--;
      select_index_y_ = kIndexFirstValue;
    ...
    for (int i = 0; i < kTableColumnNum; i++) {
      ...
      if (x1 <= mouse_x && mouse_x <= x2 && y1 <= mouse_y && mouse_y <= y2) {
        ...
        if (select_index_x_ % 2 == 0 &&
            select_index_y_ % kTableColumnNum != i) {
          PlaySoundMem(select_sound_handle_, DX_PLAYTYPE_BACK);
        }
    ...
    for (int i = 0; i < kButtonNum; i++) {
      ...
      if (x <= mouse_x && mouse_x <= x + kButtonWidth && y <= mouse_y &&
          mouse_y <= y + kButtonHeight) {
        // 変更があったときだけ音を鳴らす
  ...
  if (select_index_x_ % 2 == 0) {
    hovered_country_ =
        static_cast<LanguageRecord::Country>(select_index_y_ % kTableColumnNum);
  } else {
    hovered_country_ = current_country_;
  }
}
...
void LanguageUI::applyLanguage() {
  // 設定が変更されていなければ何もしない
  if (first_country_ == language_record_ptr_->getCurrentCountry()) {
    return;
  }
  ...
  game_setting_record_ptr.language_country =
      language_record_ptr_->getCurrentCountry();
...
void LanguageUI::drawText() const {
  ...
  // テキストの描画
  DrawStringToHandle(text_left_x, kTableTopY, attention_str0.c_str(),
                     color_back, font20_handle_);
  DrawStringToHandle(text_left_x, kTableTopY + 40, attention_str1.c_str(),
                     color_back, font20_handle_);
  DrawStringToHandle(text_left_x, kTableTopY + 80, attention_str2.c_str(),
                     color_back, font20_handle_);
}
...
void LanguageUI::drawButton() const {
  ...
  for (int i = 0; i < kButtonNum; i++) {
    ...
    const unsigned int color =
        select_index_x_ % 2 == 1 && select_index_y_ % kButtonNum == i
            ? color_hover
            : color_white;

    DrawBox(x + kButtonThickness, y + kButtonThickness,
            x + kButtonWidth - kButtonThickness,
            y + kButtonHeight - kButtonThickness, color, TRUE);
    ...
    const int text_width = GetDrawStringWidthToHandle(
        str.c_str(), static_cast<int>(str.length()), font24_handle_);

    // リセットできないときは文字を薄くする
    const unsigned int color_str =
        (i == 0 && current_country_ == first_country_) ? color_back * 2 / 3
                                                       : color_back;

    // 文字列の描画
    DrawStringToHandle(x + (kButtonWidth - text_width) / 2,
                       y + kButtonHeight / 2 - 12, str.c_str(), color_str,
                       font24_handle_);
  }
}
...
void LanguageUI::drawTable() const {
  ...
  for (int i = 0; i < kTableColumnNum; i++) {
    ...
    DrawBox(x1 + kTableThickness, y1 + kTableThickness, x2 - kTableThickness,
            y2 - kTableThickness, color_white, TRUE);

    // 1列目
    const int x3 = x2 - kTableThickness;
    const int x4 = x3 + kTableRow1Width;
    const unsigned int color =
        select_index_x_ % 2 == 0 && select_index_y_ % kTableColumnNum == i
            ? color_hover
            : color_white;
    DrawBox(x3, y1, x4, y2, color_back, TRUE);
    DrawBox(x3 + kTableThickness, y1 + kTableThickness, x4 - kTableThickness,
            y2 - kTableThickness, color, TRUE);

    // 文字列の描画
    const LanguageRecord::Country country =
        static_cast<LanguageRecord::Country>(i);
    const std::string str = string_util::EnumToStringRemoveTopK(country);
    const int text_width = GetDrawStringWidthToHandle(
        str.c_str(), static_cast<int>(str.length()), font32_handle_);
    DrawStringToHandle((x3 + x4 - text_width) / 2, y1 + 10, str.c_str(),
                       color_back, font32_handle_);

    // チェックマークの描画
    if (current_country_ == country) {
      DrawRotaGraph((x1 + x2) / 2, (y1 + y2) / 2, 0.8, 0.0, checked_img_handle_,
                    TRUE);
    }
