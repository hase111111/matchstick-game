...
class TestEntity final : public IEntity {
 public:
  ...
  inline int getLayer() const override { return layer_; }
...
TEST_CASE(
    "EntityUpdater::update when entity's update return false, should delete "
    "entity") {
  using ::match_stick::EntityUpdater;
  using ::match_stick::entity_updater_test_internal::TestEntity;
  ...
  updater.registerEntity(entity1);
  updater.registerEntity(entity2);
  CHECK_EQ(entity2.use_count(),
           2);  // entity2 is shared by updater and variable
