...
void SettingSceneUiCreator::initUI(
    const std::unique_ptr<EntityUpdater>& entity_updater_ptr) {
  // ヌルチェック
  ASSERT_NOT_NULL_PTR(entity_updater_ptr);

  // UI Base を追加
  const auto dxlib_user_interface_base_ptr =
      std::make_shared<DxLibUserInterfaceBase>(dxlib_input_ptr_);
  ...
  const auto button_ptr = std::make_shared<SimpleBoxButton>(
      language_record_ptr_, dxlib_resource_loader_ptr_, button_center_x,
      GameConst::kResolutionY - button_y_diff - button_height / 2, button_width,
      button_height, "setting_back", "data/font/azuki_font24.dft",
      [this, &entity_updater_ptr]() {
        // フェードアウト演出を追加
        const auto fade_effect_ptr = std::make_shared<FadeEffect>(
            30, FadeEffect::FadeType::kFadeOut, [this]() {
              scene_change_listener_ptr_->requestDeleteScene(
                  1, SceneChangeParameter{});
            });
  ...
  const auto apply_button_ptr = std::make_shared<SimpleBoxButton>(
      language_record_ptr_, dxlib_resource_loader_ptr_, button_center_x,
      GameConst::kResolutionY - button_y_diff * 2 - button_height * 3 / 2,
      button_width, button_height, "setting_apply_and_back",
      "data/font/azuki_font24.dft", [this, &entity_updater_ptr]() {
        // フェードアウト演出を追加
        const auto fade_effect_ptr = std::make_shared<FadeEffect>(
            30, FadeEffect::FadeType::kFadeOut,
            [this]() { scene_change_listener_ptr_->requestDeleteAllScene(); });
  ...
  const auto reset_button_ptr = std::make_shared<SimpleBoxButton>(
      language_record_ptr_, dxlib_resource_loader_ptr_, button_center_x,
      GameConst::kResolutionY - button_y_diff * 3 - button_height * 5 / 2,
      button_width, button_height, "setting_reset",
      "data/font/azuki_font24.dft", [this, &entity_updater_ptr]() {
        // フェードアウト演出を追加
        const auto fade_effect_ptr = std::make_shared<FadeEffect>(
            30, FadeEffect::FadeType::kFadeOut,
  ...
  const auto to_credit_button_ptr = std::make_shared<SimpleBoxButton>(
      language_record_ptr_, dxlib_resource_loader_ptr_, button_center_x,
      GameConst::kResolutionY - button_y_diff * 4 - button_height * 7 / 2,
      button_width, button_height, "setting_to_credit",
      "data/font/azuki_font24.dft", [this, &entity_updater_ptr]() {
        // フェードアウト演出を追加
        const auto fade_effect_ptr = std::make_shared<FadeEffect>(
            30, FadeEffect::FadeType::kFadeOut, [this]() {
              scene_change_listener_ptr_->requestAddScene(
                  SceneName::kDebug, SceneChangeParameter{});
            });
  ...
  const auto sound_change_bar_ptr = std::make_shared<SoundChangeBar>(
      language_record_ptr_, dxlib_input_ptr_, dxlib_resource_loader_ptr_,
      bgm_bar_x, bgm_bar_y);
  ...
  const auto bgm_volume_up5_button_ptr = std::make_shared<SimpleBoxButton>(
      ...
      bgm_bar_x - GameConst::kResolutionX * 57 / 192,
      bgm_bar_y - GameConst::kResolutionY * 3 / 108, button_width2,
      button_width2, "setting_volume_up", "data/font/azuki_font32.dft",
      [this]() { changeBGMVolume(5); });

  const auto bgm_volume_up_button_ptr = std::make_shared<SimpleBoxButton>(
      language_record_ptr_, dxlib_resource_loader_ptr_,
      bgm_bar_x - GameConst::kResolutionX * 47 / 192,
      bgm_bar_y - GameConst::kResolutionY * 3 / 108, button_width2,
      button_width2, "setting_volume_up", "data/font/azuki_font32.dft",
  ...
  const auto bgm_volume_down_button_ptr = std::make_shared<SimpleBoxButton>(
      ...
      bgm_bar_x + GameConst::kResolutionX * 47 / 192,
      bgm_bar_y - GameConst::kResolutionY * 3 / 108, button_width2,
      button_width2, "setting_volume_down", "data/font/azuki_font32.dft",
      [this]() { changeBGMVolume(-1); });
  ...
  const auto sound_effect_volume_up5_button_ptr =
      std::make_shared<SimpleBoxButton>(
          language_record_ptr_, dxlib_resource_loader_ptr_,
          bgm_bar_x - GameConst::kResolutionX * 57 / 192,
          bgm_bar_y + GameConst::kResolutionY * 15 / 108, button_width2,
          button_width2, "setting_volume_up", "data/font/azuki_font32.dft",
          [this]() { changeSEVolume(5); });

  const auto sound_effect_volume_up_button_ptr =
      std::make_shared<SimpleBoxButton>(
          language_record_ptr_, dxlib_resource_loader_ptr_,
          bgm_bar_x - GameConst::kResolutionX * 47 / 192,
          bgm_bar_y + GameConst::kResolutionY * 15 / 108, button_width2,
          button_width2, "setting_volume_up", "data/font/azuki_font32.dft",
          [this]() { changeSEVolume(1); });

  const auto sound_effect_volume_down_button_ptr =
      std::make_shared<SimpleBoxButton>(
          language_record_ptr_, dxlib_resource_loader_ptr_,
          bgm_bar_x + GameConst::kResolutionX * 47 / 192,
          bgm_bar_y + GameConst::kResolutionY * 15 / 108, button_width2,
  ...
  const auto sound_effect_volume_down5_button_ptr =
      std::make_shared<SimpleBoxButton>(
          language_record_ptr_, dxlib_resource_loader_ptr_,
          bgm_bar_x + GameConst::kResolutionX * 57 / 192,
          bgm_bar_y + GameConst::kResolutionY * 15 / 108, button_width2,
  ...
  dxlib_user_interface_base_ptr->registerInterface(bgm_volume_up5_button_ptr,
                                                   6);
  dxlib_user_interface_base_ptr->registerInterface(bgm_volume_up_button_ptr, 7);
  dxlib_user_interface_base_ptr->registerInterface(bgm_volume_down_button_ptr,
                                                   8);
  dxlib_user_interface_base_ptr->registerInterface(bgm_volume_down5_button_ptr,
                                                   9);
  dxlib_user_interface_base_ptr->registerInterface(
      sound_effect_volume_up5_button_ptr, 10);
  dxlib_user_interface_base_ptr->registerInterface(
      sound_effect_volume_up_button_ptr, 11);
  dxlib_user_interface_base_ptr->registerInterface(
      sound_effect_volume_down_button_ptr, 12);
  dxlib_user_interface_base_ptr->registerInterface(
      sound_effect_volume_down5_button_ptr, 13);
