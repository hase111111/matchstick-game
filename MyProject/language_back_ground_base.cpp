...
LanguageBackGroundBase::LanguageBackGroundBase(
    ...
    : title_text_(lang->getValue("language_title")),
      font_handle_(dxlib_resource_loader_ptr->getFontHandle(
          lang->getCurrentCountry(), "data/font/azuki_font48.dft")),
      icon_handle_(dxlib_resource_loader_ptr->getImageHandle(
          "data/img/icon_language.png")) {
  // ポインタのチェック
...
void LanguageBackGroundBase::draw() const {
  ...
  // 下地
  DrawBox(0, 0, GameConst::kResolutionX, GameConst::kResolutionY, color_white,
          TRUE);
  ...
  // タイトル
  DrawStringToHandle(icon_pos_x + 50, icon_pos_y - 25, title_text_.c_str(),
                     color_back, font_handle_, color_white);
}
