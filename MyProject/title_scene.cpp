...
TitleScene::TitleScene(
  ...
  entity_updater_ptr_->registerEntity(
      std::make_shared<TitleBackGroundBase>(dxlib_resource_loader_ptr));
  ...
  entity_updater_ptr_->registerEntity(
      std::make_shared<TitleHandAnimation>(dxlib_resource_loader_ptr));
...
void TitleScene::draw() const { entity_updater_ptr_->draw(); }
...
void TitleScene::callbackForSceneChange() {
  auto scene_change_func = [this]() {
    scene_change_listener_ptr_->requestAddScene(SceneName::kMenu,
                                                SceneChangeParameter{});
  };

  auto fade_effect_ptr = std::make_shared<FadeEffect>(
      30, FadeEffect::FadeType::kFadeOut, scene_change_func);
