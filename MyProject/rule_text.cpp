...
RuleText::RuleText(
    ...
    : rule_text_(createRuleText(language_record_ptr)),
      need_hyphen_(
          LanguageRecord::isEnglish(language_record_ptr->getCurrentCountry())),
...
void RuleText::update(const int number) {
  ...
  hexagon_angle_velocity_ =
      (hexagon_angle_velocity_ > 0.0)
          ? hexagon_angle_velocity_ - MathConst<double>::kPi * 0.05
          : 0.0;
}
...
void RuleText::drawHexagon(const int center_x, const int center_y,
                           const int radius, const bool fill,
                           const double angle_offset,
                           const unsigned int edge_color,
                           const unsigned int fill_color) const {
  ...
  const float vertex_x[vertex_num] = {
      static_cast<float>(center_x +
                         radius * cos(angle * 0 * MathConst<double>::kPi / 180 +
                                      angle_offset)),
      static_cast<float>(center_x +
                         radius * cos(angle * 1 * MathConst<double>::kPi / 180 +
                                      angle_offset)),
      static_cast<float>(center_x +
                         radius * cos(angle * 2 * MathConst<double>::kPi / 180 +
                                      angle_offset)),
      static_cast<float>(center_x +
                         radius * cos(angle * 3 * MathConst<double>::kPi / 180 +
                                      angle_offset)),
      static_cast<float>(center_x +
                         radius * cos(angle * 4 * MathConst<double>::kPi / 180 +
                                      angle_offset)),
      static_cast<float>(center_x +
                         radius * cos(angle * 5 * MathConst<double>::kPi / 180 +
                                      angle_offset))};

  const float vertex_y[vertex_num] = {
      static_cast<float>(center_y +
                         radius * sin(angle * 0 * MathConst<double>::kPi / 180 +
                                      angle_offset)),
      static_cast<float>(center_y +
                         radius * sin(angle * 1 * MathConst<double>::kPi / 180 +
                                      angle_offset)),
      static_cast<float>(center_y +
                         radius * sin(angle * 2 * MathConst<double>::kPi / 180 +
                                      angle_offset)),
      static_cast<float>(center_y +
                         radius * sin(angle * 3 * MathConst<double>::kPi / 180 +
                                      angle_offset)),
      static_cast<float>(center_y +
                         radius * sin(angle * 4 * MathConst<double>::kPi / 180 +
                                      angle_offset)),
      static_cast<float>(center_y +
                         radius * sin(angle * 5 * MathConst<double>::kPi / 180 +
                                      angle_offset))};
...
void RuleText::drawNumberBox() const {
  const int center_x = box_left_ + GameConst::kResolutionX / 20;
  const int center_y = box_top_ + GameConst::kResolutionY / 12;
  const int radius = 35;
  const double angle_offset =
      counter_ * MathConst<double>::kPi / 180 - hexagon_angle_velocity_;
  const unsigned int edge_color = GetColor(0, 0, 0);
  const unsigned int fill_color = GetColor(255, 255, 255);

  drawHexagon(center_x, center_y, radius, false, angle_offset, edge_color,
              fill_color);

  // 現在の number を表示
  const std::string text = std::to_string(number_ + 1);
  const int text_width = GetDrawStringWidthToHandle(
      text.c_str(), static_cast<int>(text.size()), font32_handle_);
  const int text_x = center_x - text_width / 2;
  const int text_y = center_y - 16;

  DrawStringToHandle(text_x, text_y, text.c_str(), GetColor(0, 0, 0),
                     font32_handle_);
}
...
void RuleText::drawText() const {
  ...
  const int title_width = GetDrawStringWidthToHandle(
      title.c_str(), static_cast<int>(title.size()), font32_handle_);
  ...
  for (int i = 1; i < draw_text_.size(); ++i) {
    ...
    DrawStringToHandle(box_left_ + 40, box_top_ + 75 + 35 * i, text.c_str(),
                       GetColor(0, 0, 0), font24_handle_);
  }
}
