...
std::shared_ptr<SceneStack> GameMainLoop::initializeSceneStack() const {
  ...
  const auto language_record_ptr =
      std::make_shared<LanguageRecord>(language_record_initializer.initialize(
          game_setting_record_ptr_->language_country));

  const auto dxlib_resource_loader_ptr =
      std::make_shared<DxLibResourceLoader>();
  dxlib_resource_loader_ptr->changeAllSEVolume(
      game_setting_record_ptr_->sound_volume);

  auto scene_creator_ptr = std::make_unique<SceneCreator>(
      scene_change_listener_ptr_, fps_controller_ptr_, language_record_ptr,
      dxlib_input_ptr_, dxlib_resource_loader_ptr);
