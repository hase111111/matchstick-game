...
DxLibUserInterfaceBase::DxLibUserInterfaceBase(
...
void DxLibUserInterfaceBase::registerInterface(
  ...
  ASSERT(dxlib_interfaces_.find(id) == dxlib_interfaces_.end(),
         std::format("id ({}) duplicated", id));
...
void DxLibUserInterfaceBase::registerInterfaceDeployment(int target_id,
                                                         int up_id, int down_id,
                                                         int left_id,
                                                         int right_id) {
  // すでに登録されているならば，警告を出す
  ...
  ui_deployment_.id_to_direction_map[target_id] = {up_id, down_id, left_id,
                                                   right_id};
}
...
void DxLibUserInterfaceBase::updateSelectedIdWhenMouseUsed() {
  ...
  for (const auto& [id, dxlib_interface] : dxlib_interfaces_) {
    if (dxlib_interface->isHovered(dxlib_input_->getCursorPosX(),
                                   dxlib_input_->getCursorPosY())) {
      selected_id_ = id;
      return;
    }
  }
...
void DxLibUserInterfaceBase::updateSelectedIdWhenKeyboardUsed() {
  ...
  // selected_id_ の値の UI
  // が存在しているか確かめ，なければデフォルト値に指定する
  if (!dxlib_interfaces_.contains(selected_id_)) {
    DEBUG_PRINT_ERROR(std::format("UI deployment error, selected_id_{} is none",
                                  selected_id_));
